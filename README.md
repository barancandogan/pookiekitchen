...
a chapter's line or a dish's, with its correction. `correctSeed()` applies it
wherever a value still reads exactly as the old seed did, both when the site
is built and when the panel loads. Anything the owner has edited is left
alone. Each chapter's line ("Never fried, never held.") has its own field in
the panel's menu editor.
